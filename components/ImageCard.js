...
import { trickleUpdateObject } from "./path-to-trickleUpdateObject" // Import trickleUpdateObject

function ImageCard({ image, onImageClick, viewMode = "grid" }) {
  const [likes, setLikes] = React.useState(image.objectData?.likes || Math.floor(Math.random() * 100))
  const [liked, setLiked] = React.useState(false)
  const [views, setViews] = React.useState(image.objectData?.views || Math.floor(Math.random() * 500))
  const [isLoading, setIsLoading] = React.useState(true)

  if (!image || !image.objectData) return null
  ...
  const handleLike = async (e) => {
    ...
    // Aquí podrías actualizar en la base de datos
    try {
      await trickleUpdateObject("image", image.objectId, {
        ...data,
  ...
  const handleDownload = (e) => {
    ...
    link.download = `${data.title || "image"}.jpg`
    link.target = "_blank"
      ...
      >
        <div className="flex space-x-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {data.description && (
              <p className="text-[var(--text-secondary)] text-sm mb-3 line-clamp-2">{data.description}</p>
            )}
      ...
      <div className="p-4">
        ...
        {normalizedTags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-3">
            ...
            {normalizedTags.length > 3 && (
              <span className="text-[var(--text-secondary)] text-xs px-2 py-1">+{normalizedTags.length - 3}</span>
            )}
          </div>
        )}
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[var(--text-secondary)] font-medium">{data.userName || "Usuario Anónimo"}</span>
          </div>
